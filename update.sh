...
for optimized in debian alpine; do
  ...
  echo "| DockerHub image | Dockerfile | OS | Postgres | PGQ | PGQD |" >>_dockerlists_${optimized}.md
  echo "| --------------- | ---------- | -- | -------- | --- | ---- |" >>_dockerlists_${optimized}.md
done
...
defaultAlpinePgqVersion='3.5.1'
defaultAlpinePgqdVersion='3.5'
defaultAlpineSuite='3.20'
...
for version in "${versions[@]}"; do
  ...
  pgqdPackageName="pgqd"
  pgqdFullVersion="$(echo "$versionList" | awk -F ': ' '$1 == "Package" { pkg = $2 } $1 == "Version" && pkg == "'"$pgqdPackageName"'" { print $2; exit }' || true)"
  echo "pgqdPackageName=${pgqdPackageName}"
  echo "pgqdFullVersion=${pgqdFullVersion}"
  ...
  if [ -z "$pgqFullVersion" ]; then
    ...
    (
      ...
      sed -i 's/%%TXT_AUTOGENERATED%%/'"$autogenerated"'/g; s/%%PG_MAJOR%%/'$postgresVersion'/g; s/%%PGQ_VERSION%%/'$pgqFullVersion'/g; s/%%PGQD_VERSION%%/'$pgqdFullVersion'/g; s/%%DEBIAN_VERSION%%/'"$suite"'/g;' "$version/Dockerfile"
  ...
  pgqSrcVersion="${pgqFullVersion%%-*}"
  pgqdSrcVersion="${pgqdFullVersion%%-*}"
  ...
  for variant in alpine; do
    ...
    (
      ...
      sed -i 's/%%TXT_AUTOGENERATED%%/'"$autogenerated"'/g; s/%%PG_MAJOR%%/'"$postgresVersion"'/g; s/%%PGQ_VERSION%%/'"$defaultAlpinePgqVersion"'/g; s/%%PGQD_VERSION%%/'"$defaultAlpinePgqdVersion"'/g; s/%%PGQ_SHA256%%/'"$pgqSha256"'/g; s/%%PGQD_SHA256%%/'"$pgqdSha256"'/g;' "$version/$variant/Dockerfile"

      echo "| [zkoesters/postgres-pgq:${version}-${variant}](${dockerhublink}${version}-${variant}) | [Dockerfile](${githubrepolink}/${version}/${variant}/Dockerfile) | alpine:${defaultAlpineSuite} | ${postgresVersion} | ${pgqDocSrc} |" >>_dockerlists_${optimized}.md
    )
  done
done
